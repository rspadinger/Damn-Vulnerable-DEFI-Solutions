...
describe("[Challenge] The rewarder", function () {
    ...
    before(async function () {
        ...
        expect(
            await accountingToken.hasAllRoles(rewarderPool.address, minterRole | snapshotRole | burnerRole)
        ).to.be.true
    ...
    it("Execution", async function () {
        ...
        //advance 5 days to start round 3
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]) // 5 days

        const attackerFactory = await ethers.getContractFactory("RewarderAttacker")
